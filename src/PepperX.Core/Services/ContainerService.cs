namespace PepperX.Core.Services
{
    ...
    public sealed class ContainerService
    {
        ...
        public async Task<ContainerResponse> CreateAsync(ContainerCreateRequest request, CancellationToken token = default)
        {
            ...
            using Activity? __act = PepperXTelemetry.StartActivity("container.create", ActivityKind.Internal);
            bool __ok = true;
            try
            {
            ...
            {
                PepperXTelemetry.RecordContainer("create", Stopwatch.GetElapsedTime(__ts).TotalSeconds, __ok);
            }
        }
        ...
        {
            long __ts = Stopwatch.GetTimestamp();
            using Activity? __act = PepperXTelemetry.StartActivity("container.read", ActivityKind.Internal);
            bool __ok = true;
            try
            {
            ...
            {
                PepperXTelemetry.RecordContainer("read", Stopwatch.GetElapsedTime(__ts).TotalSeconds, __ok);
            }
        }
        ...
        {
            long __ts = Stopwatch.GetTimestamp();
            using Activity? __act = PepperXTelemetry.StartActivity("container.exists", ActivityKind.Internal);
            bool __ok = true;
            try
            {
                return await _Db.Containers.ExistsAsync(name, token).ConfigureAwait(false);
            }
            catch (Exception __ex)
            {
                __ok = false;
            ...
            {
                PepperXTelemetry.RecordContainer("exists", Stopwatch.GetElapsedTime(__ts).TotalSeconds, __ok);
            }
        }
        ...
        {
            long __ts = Stopwatch.GetTimestamp();
            using Activity? __act = PepperXTelemetry.StartActivity("container.enumerate", ActivityKind.Internal);
            bool __ok = true;
            try
            {
            ...
            {
                PepperXTelemetry.RecordContainer("enumerate", Stopwatch.GetElapsedTime(__ts).TotalSeconds, __ok);
            }
        }
        ...
        {
            long __ts = Stopwatch.GetTimestamp();
            using Activity? __act = PepperXTelemetry.StartActivity("container.update_tags", ActivityKind.Internal);
            bool __ok = true;
            try
            {
            ...
            {
                PepperXTelemetry.RecordContainer("update_tags", Stopwatch.GetElapsedTime(__ts).TotalSeconds, __ok);
            }
        }
        ...
        {
            long __ts = Stopwatch.GetTimestamp();
            using Activity? __act = PepperXTelemetry.StartActivity("container.set_resp_index", ActivityKind.Internal);
            bool __ok = true;
            try
            {
                Container container = await RequireAsync(name, token).ConfigureAwait(false);
            ...
            {
                PepperXTelemetry.RecordContainer("set_resp_index", Stopwatch.GetElapsedTime(__ts).TotalSeconds, __ok);
            }
        }
        ...
        {
            long __ts = Stopwatch.GetTimestamp();
            using Activity? __act = PepperXTelemetry.StartActivity("container.set_multipart_expiry", ActivityKind.Internal);
            bool __ok = true;
            try
            {
                Container container = await RequireAsync(name, token).ConfigureAwait(false);
            ...
            {
                PepperXTelemetry.RecordContainer("set_multipart_expiry", Stopwatch.GetElapsedTime(__ts).TotalSeconds, __ok);
            }
        }
        ...
        {
            long __ts = Stopwatch.GetTimestamp();
            using Activity? __act = PepperXTelemetry.StartActivity("container.read_cache", ActivityKind.Internal);
            bool __ok = true;
            try
            {
                Container container = await RequireAsync(name, token).ConfigureAwait(false);
            ...
            {
                PepperXTelemetry.RecordContainer("read_cache", Stopwatch.GetElapsedTime(__ts).TotalSeconds, __ok);
            }
        }
        ...
        {
            long __ts = Stopwatch.GetTimestamp();
            using Activity? __act = PepperXTelemetry.StartActivity("container.update_cache", ActivityKind.Internal);
            bool __ok = true;
            try
            {
                if (request == null) throw new ArgumentNullException(nameof(request));
            ...
            {
                PepperXTelemetry.RecordContainer("update_cache", Stopwatch.GetElapsedTime(__ts).TotalSeconds, __ok);
            }
        }
        ...
        {
            long __ts = Stopwatch.GetTimestamp();
            using Activity? __act = PepperXTelemetry.StartActivity("container.delete", ActivityKind.Internal);
            bool __ok = true;
            try
            {
                Container container = await RequireAsync(name, token).ConfigureAwait(false);
            ...
            {
                PepperXTelemetry.RecordContainer("delete", Stopwatch.GetElapsedTime(__ts).TotalSeconds, __ok);
            }
        }
