...
docker pull jchristn77/pepperx-server:%VERSION_TAG%
if errorlevel 1 (
    echo Failed to pull jchristn77/pepperx-server:%VERSION_TAG% into the local store.
    exit /b %errorlevel%
)
docker pull jchristn77/pepperx-server:latest
if errorlevel 1 (
    echo Failed to pull jchristn77/pepperx-server:latest into the local store.
    exit /b %errorlevel%
)
